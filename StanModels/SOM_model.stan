functions {
  vector post_thetacdf (
    ...
    vector[localN] out;
    out = lambda*(ES - cost * EC);
    if(degrees[1]==0){out[1] =  negative_infinity();}
    for(j in 2:localN){
...
data {
  ...
  //real prior_ccdf_array[Nsubj,Smax,Kmax]; // ccdf of the prior at each degree (with special condition when degree is 1)
...
transformed parameters {
  ...
  for (n in 1:Nsubj){
    for (s in 1:S[n]){
      ...
      local_theta_cdf = post_thetacdf(local_prior_length,EC_array[n,s][1:local_prior_length],ES_array[n,s][1:local_prior_length],full_degree_array[n,s,1:local_prior_length],subj_lambda[n],mean_cost+subj_cost[n]+adj_cost[adjective[n,s]]);
...
model {
  ...
  subj_lambda_sigma ~ gamma(1.3,3.0);
