functions {
  vector post_thetacdf (
    ...
    vector[localN] out;
    out[1] = degrees[1]==0 ? 0 : inv_logit(-lambda*cost)*prior[1];
    for(i in 1:(localN-1)){
      out[i+1] = prior[i+1]*prior_ccdf_star[i]*inv_logit(-lambda*(log(prior_ccdf_star[i])+cost));
    }
...
data {
  ...
  array[Nsubj,Smax,20] real prior_array; // ccdf of the prior at each degree (with special condition when max degree is 1)
...
transformed parameters {
  ...
  for (n in 1:Nsubj){
    for (s in 1:S[n]){
      ...
      local_theta_cdf = post_thetacdf(prior_length_array[n,s],to_vector(prior_ccdf_array[n,s,1:prior_length_array[n,s]]),prior_array[n,s,1:prior_length_array[n,s]],full_degree_array[n,s,1:prior_length_array[n,s]],subj_lambda[n],mean_cost+subj_cost[n]+adj_cost[adjective[n,s]]);
...
model {
  ...
  subj_lambda_sigma ~ gamma(1.5,3.0);
